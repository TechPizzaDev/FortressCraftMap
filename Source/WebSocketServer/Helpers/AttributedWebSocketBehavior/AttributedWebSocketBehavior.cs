...
namespace WebSocketServer
{
    public abstract partial class AttributedWebSocketBehavior : WebSocketBehavior
    {
        ...
        /// <returns></returns>
        protected bool Missing(object token)
        {
            if (token == null || (token is IList list && list.Count == 0))
            {
        ...
        /// <returns></returns>
        protected bool Missing<T>(object token, string name, out T value)
        {
            if (token is IDictionary<string, object> dic)
                if (dic.TryGetValue(name, out object raw))
                    if (raw is T typed)
                    {
                        value = typed;
                        return false;
        ...
        /// <param name="value">The found value in the token.</param>
        /// <returns></returns>
        protected bool Missing<T>(object token, int index, out T value)
        {
            if (token is IList<object> list)
                if (index < list.Count)
                {
